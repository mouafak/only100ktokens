...
const ClaimButton = () => {
  ...
  const handleClaim = async () => {
    ...
    console.log(`[${claimId}] Starting claim process`);

    // Check if the user has an unconfirmed claim transaction
    const hasUnconfirmedClaim = await checkUnconfirmedClaim();
    if (typeof hasUnconfirmedClaim === 'boolean' && hasUnconfirmedClaim) {
      console.log(`[${claimId}] User has an unconfirmed claim transaction`);
      return;
    }
    ...
    try {
      ...
      //  1. Prepare the transaction client-side
      const preparedTransaction = await prepareTransaction({ userWallet });
      if (!preparedTransaction.success) {
        toast.error(
          'Failed to prepare transaction: ' + preparedTransaction.message,
          { id: 'claiming-tokens' }
        );
        setIsLoading(false);
        return;
      ...
      //  3. Sign the transaction by the user
      if (!isSolanaWallet(primaryWallet)) {
        console.error('Primary wallet is not a Solana wallet');
        toast.error('Wallet connected is not a Solana wallet', {
          id: 'claiming-tokens',
        });
        setIsLoading(false);
        return;
      ...
      console.error('Error occurred while claiming:', error);
    } finally {
      setIsLoading(false);
    }
  };
